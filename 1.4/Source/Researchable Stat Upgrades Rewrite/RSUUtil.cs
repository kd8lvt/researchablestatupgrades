...
namespace ResearchableStatUpgrades
{
    public static class RSUUtil
    {
		...
		{
			List<ResearchMod> list = (List<ResearchMod>)RSUUtil.researchModsField.GetValue(def);
			return (list != null) ? list.Find((ResearchMod m) => m.GetType().IsInst(type)) : null;
		}

		public static T GetResearchMod<T>(this ResearchProjectDef def) where T : ResearchMod
		{
			return (T)((object)def.GetResearchMod(typeof(T)));
		}
		...
		public static readonly FieldInfo researchModsField = typeof(ResearchProjectDef).GetField("researchMods", RSUUtil.universal);
	}
}